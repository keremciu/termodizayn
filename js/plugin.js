...
(function ($, undefined) {
	...
	$.extend(Selectbox.prototype, {
		...
		_attachSelectbox: function (target, settings) {
			...
			sbToggle = $("<svg id=sbToggle_"+inst.uid+" class='icon "+inst.settings.classToggle+"'><use xlink:href='#icon-arrow-drop-down'>", {
				"click": function (e) {
					e.preventDefault();
					closeOthers.apply($(this), []);
					var uid = $(this).attr("id").split("_")[1];
		...
		_openSelectbox: function (target) {
			...
			$.data(target, PROP_NAME, inst);
			$("#sbHolder_" + inst.uid).addClass('isActive');
		},
		/**
