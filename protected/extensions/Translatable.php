 ...
 * @contributor Kerem Sevencan <info@kerem.ws>
 */
class Translatable extends CActiveRecordBehavior
{
     ...
     */
    public $translationRelation = 'Translates';

    public $translationTable = 'category';

    /**
     * @var string name of language column in translation table. Default is 'language'.
     */
    public $languageColumn = 'lang_id';
    ...
    {
        $tablename = $this->owner->model()->tableSchema->name;
        $table = $this->translationTable;

        //$event->sender->tableSchema->name;
        if($this->_allQueryLanguages===null)
        {
            $language   = $this->_queryLanguage===null ? $this->getLanguage() : $this->_queryLanguage;
            $on         = "{$this->translationRelation}.{$this->languageColumn}='$language' AND {$this->translationRelation}.reference_table='$table'";
        }
        else
        {
    ...
    {
        $behavior = $this->owner->model()->asa(get_class($this));
        $tablename = $this->owner->model()->tableSchema->name;
        $ties = $this->translationRelation;
        $relation = $this->owner->metaData->relations[$ties];
        $class = $relation->className;
        $pkName = $relation->foreignKey;

        $lab = $this->owner->getRelated($ties);

        $this->_language = $behavior->_queryLanguage;
        $behavior->_queryLanguage = null;

        if (Yii::app()->sourceLanguage != $this->_language) {
            foreach ($this->owner->$ties as $key => $translate) {
                if ($translate->reference_table == $tablename) {
                    $this->owner->$key = $translate->value;
                }
            }
        }
