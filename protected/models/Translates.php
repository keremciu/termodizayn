...
class Translates extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('value, original_text', 'required'),
			array('reference_id, is_published', 'numerical', 'integerOnly'=>true),
			array('reference_table, reference_field', 'length', 'max'=>100),
			array('original_value', 'length', 'max'=>255),
			array('modified_by', 'length', 'max'=>11),
			array('modified', 'safe'),
			array('id, lang_id, reference_id, reference_table, reference_field, value, original_value, original_text, modified, modified_by, is_published', 'safe', 'on'=>'search'),
		);
	}
