...
class TbButtonColumn extends CButtonColumn {
	 ...
	 */
	public $viewButtonIcon = 'visibility';
	...
	protected function renderButton($id, $button, $row, $data) {
		...
		if (!isset($options['data-toggle'])) {
			//$options['data-toggle'] = 'bs.tooltip';
		}
		...
		if (isset($button['icon']) && $button['icon']) {
			...
			echo CHtml::link('<svg class="td-icon td-icon-'.$button['icon'].'"><use xmlns:xlink="http://www.w3.org/1999/xlink" xlink:href="#icon-'.$button['icon'].'"></use></svg>', $url, $options);
